...
class my_ResNet(nn.Module):
    ...
    def __init__(self,architecture = 50, pretrained = False, num_classes = 11, dropout = .5):

        super(my_ResNet, self).__init__()

        if(architecture == 50):
            #self.resnet_model = ResNet(Bottleneck, blocks = [3, 4, 6, 3])
            self.resnet_model = ResNet(Bottleneck, blocks = [2, 2, 6, 3])
        ...
        #self.resnet_model.fc = nn.Linear(num_ftrs, num_ftrs * 2)
        #self.resnet_model.layer3 = nn.Identity()
        #self.resnet_model.layer4 = nn.Identity()
        #self.resnet_model.fc = nn.Identity()
        
        self.resnet_model.fc = nn.Sequential(

            nn.Linear(512 * 5 * 5, 4096, bias = True),
            nn.ReLU(inplace = False),
